...
const Tetris: React.FC = () => {
    ...
    function rotateTetromino() {
        ...
        if (isColliding(position, rotatedShape)) {
            if (position.x + rotatedShape[0].length > COLS) {
                newPosition.x -= position.x + rotatedShape[0].length - COLS;
            } else if (position.x < 0) {
                newPosition.x = 0;
            }
        }

        if (!isColliding(newPosition, rotatedShape)) {
            setPosition(newPosition);
            ...
            for (let x = 0; x < shape[y].length; x++) {
                if (shape[y][x]) {
                    ...
                    if (newY >= 0) {
                        ...
                        createMergeParticles(newX, newY);
                    }
                }
            }
        }
    ...
    return (
        <div className="game-section">
            {gameOver ? (
                <h2>Guess what, u lose</h2>
            ) : (
                <>  
                    <div className="">
                        W | ArrowUp : Rotate
                        <br />
                        S | ArrowDown : Drop Down
                        <br />
                        A | ArrowLeft : Left
                        <br />
                        D | ArrowRight : Right
